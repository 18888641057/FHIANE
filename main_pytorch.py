...
def train_model(file_name,network_data):   #training_data_by_type：{‘edge_type'}=[[nodex,nodey]...]
    ...
    csloss1 = CSLoss(num_nodes, num_sampled, embedding_size)
    csloss2  =CSLoss(num_nodes, num_sampled, embedding_size)
    ...
    optimizer = torch.optim.Adam([{"params": structure_model.parameters()},{"params": model.parameters()},
                                  {"params": nsloss2.parameters()},{"params": nsloss1.parameters()},
                                   {"params": csloss1.parameters()},{"params": csloss2.parameters()},
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(data_iter):
            ...
            loss = nsloss1(data[0].to(device), content_embs[0], data[1].to(device))\
                   + nsloss2(data[0].to(device), structure_embs[0], data[1].to(device)) \
                    + csloss1(data[0].to(device), structure_embs[0], structure_embs[2],data[1].to(device))\
                    + csloss2(data[0].to(device), content_embs[0], structure_embs[2], data[1].to(device))
        ...
        with torch.no_grad():
            ...
            if cur_score > best_score:
                ...
                filepath = 'model/' + args.input.split('/')[-1] + '/MyModel'
